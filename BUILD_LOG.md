...
**Tests:** 16 more than PR A, 1866 in the suite, 144.1 s. `ruff check src tests evals` clean, `ruff format --check` clean on every changed `.py`, mypy strict clean on 88 source files. With torch hidden behind an import hook the eight touched test files give 425 passed and 15 skipped, every skip with "torch is not installed", which is CI's shape; the Run All check also skips where nbclient is absent. No pinned or byte-identical test moved.
