...
## Sprint 4: v0.6, DLModelTarget vision. Sun 2026-09-13 to Sat 2026-09-19, release Sun 2026-09-20 (held that evening, released 2026-09-21)
...
| Sun Sep 20 | RTX 4050 validation, the public claim: install over WSL2, same example, CUDA versus MPS epoch time, VRAM peak, an OOM run to prove capture, and the example READMEs updated with the 4050 numbers. Tony's certification run on the vision example with measured headroom (sequential, Ollama restarted first), then **release v0.6.0** | validation notes, v0.6.0 out | Day 7's work ran today, with a cut line: PRs #72, #73 and #74 merged; the keep-best guard for image sessions: #76 merged, main 7e0c8b9; the re-certification run: laptop_price rmse 411.89 to 292.38 in 6 iterations, 73.3% of headroom against 56.2% at v0.5, no traceback; the prompt run: tweet_irony, 2 iterations, no traceback, cells confined, the researcher on the prompt wording, f1 0.7652 at baseline then 0.7544 and 0.7652, so no candidate beat it; Flowers102 on the keep-best code: accuracy 0.5544 to 0.9811, 3 iterations, 39 min, `KEPT` printed twice, both on ties; the RTX 4050 validation: CUDA seen, the baseline trained on the GPU, out of memory caught cleanly, no experiment ran because the 4B driver never made the tool call; Tony's certification run: EuroSAT on the released code, 2026-09-21: the baseline reproduced the stored 0.9496 exactly and iteration 1 reached **0.9876** (resnet18, 128 px, 10 epochs), above the 0.9848 this release was certified at; stopped on patience after 4, no traceback, and `load(best_model.pt).predict()` re-scored 0.9876 over all 5,400 sealed images; then the release, see the release entry |
...
## Sprint 5: v0.7, cost-constrained serving. Fri 2026-09-25 to Sat 2026-10-10, release Sun 2026-10-11

*(Re-dated 2026-09-25 at the Day 1 plan gate, four days after v0.6.0 shipped, and again on 2026-09-26 when Tony set the ideal for the release: the Pricer fetches live prices, and the Researcher and the Pricer go back and forth until a model fits the budget. The August dates below it were set before v0.5 waited on its certification run. A row is a calendar day and a day can carry several PRs. Tony's calls at the gates: the Claude API adapter stays in v1.0 with the benchmark; quantization moves to the backlog, since no full agent run has finished on CUDA; the price gate names the first entry on the ready line that fits, never the cheapest; a static price list is not where this can stay, so live prices come before the wall; release Sun Oct 11 rather than Oct 4, so the feature meets a live run before it ships.)*
...
| Date | Focus | Lands | Done? |
|---|---|---|---|
| Fri Sep 25 to Sat Sep 26 | **Day 1: the winner comes with its serving price.** A dated price snapshot in the package (13 machines across AWS, GCP and Azure, the API models a prompt run can name, every row sourced), the Pricer, facts per family (a table by the estimator classes on the last error-free cell up to the one that wrote the predictions, an image by its recipe through the layer grammar, a prompt by its model and the tokens it spent per record, which the kernel now keeps), `--requests-per-hour`, the `serving:` line after `best:` and the `serving` block in `best.json` and `prompts.yaml`. No budget, no refusal, no prompt change | `schemas/serving.py`, `core/serving.py`, `core/serving_prices.json`, codegen token keys, `deliver/prompt_record.py`, cli, `llm/factory.py`, tests, RESEARCH_LOG, LIMITATIONS, DECISIONS, README | built Fri Sep 25 evening into Sat Sep 26; see the Day 1 entry |
| Sat Sep 26 | **Day 2: live prices.** The Pricer fetches the clouds' own machine-readable price lists at run time (AWS's bulk file per region and Azure's open API need no key; GCP through a user key, else the shipped file), caches them under the config folder with a date, and falls back to the last cache and then the shipped snapshot when offline, printing which one it used; `iterate prices refresh`; whole instance families instead of 13 rows; `--cloud` and `--region`, the default comparing AWS and GCP with the cheapest first; capacity for machines never measured scaled by vCPU from the 2-vCPU rate, said out loud; the sizes of every model in `timm_models.txt` from timm's published results table, fetched and cached the same way, so a network the Researcher names can be priced | `core/serving.py`, `core/prices.py`, `core/timm_sizes.csv`, `schemas/serving.py`, `cli.py`, tests, RESEARCH_LOG, LIMITATIONS, README | built Sat Sep 26; see the Day 2 entry. Four deltas from this row: AWS is fetched only by `iterate prices refresh` (its file is 300 MB) and Azure in the background with `--cloud azure`; GCP through a key was cut; the cache is `~/.cache/iterate/prices/`, not the config folder; 926 of the 1,101 names in `timm_models.txt` are in timm's table, not every one |
| Sun Sep 27 | **Day 3: the wall.** `--serving-budget`; every candidate priced before it is briefed; over-budget findings listed with their price but off the ready line; a brief naming one refused by the price gate, which names the first entry on the ready line, top down, that fits; refusals remembered on the run's history; a baseline over budget refused before the loop; one sentence in the supervisor prompt, probed on gemma4:12b | `core/serving.py`, `core/supervisor.py`, `core/vision_levers.py`, `core/ledger.py`, prompts.yaml, tests | built Sat Sep 26; see the Day 3 entry. Deltas from this row: the sentence lives in the over-budget block, present only with a budget, not in the system prompt, so a run without one sends the same bytes; `core/ledger.py` was not touched, and `core/agent_loop.py`, `cli.py` and `schemas/serving.py` were; a finished experiment over the budget is stamped and never the best on every family, which the row did not say; refusals are remembered on the wall's own list and on the history row; the baseline refusal prices from what is in hand before a folder exists |
| Mon Sep 28 | **Day 4: the Researcher and the Pricer go back and forth.** When every finding is over budget the harness asks the Researcher for the next best models with "not these" and no cost in the question, the Pricer judges again, up to a cap of rounds; when the Researcher runs dry the run says so instead of briefing a model it cannot serve; a live image run on gemma4:12b with a budget that refuses the first findings | `core/researcher.py`, `core/agent_loop.py`, `core/serving.py`, prompts.yaml, tests, a live check | |
| Tue Sep 29 | **Day 5: `iterate cost`.** Every driver call's usage summed per role and per experiment, `cost_usd` filled from the same price table, the command reads a run's db; operating cost reported, never a constraint | `llm/`, `core/agent_loop.py`, `cli.py`, tests | |
| Wed Sep 30 | **Day 6: cheaper prompt runs (carry-in 1).** Variants ranked on a 25-record slice, the survivor promoted to the full slice before submit; replayed on the recorded runs before it ships | `core/prompt_runtime.py`, `targets/prompt.py`, prompts.yaml, tests | |
| Thu Oct 1 | **Day 7: the demo.** The same task with and without a budget gives two recommendations: an image run whose budget refuses the first findings and the loop finds one that fits, a prompt run on a priced API target, a table run | example, RESULTS, README | |
| Fri Oct 2 | **Day 8: the fix round** from the live runs, plus the cheap v0.6.1 rows: the lost-try guard refusing a marker the best carries, the cheap-cell cap, the refused-run test that reads the working folder's `.env`, the three dev one-liners | fixes, tests | |
| Sat Oct 3 to Fri Oct 9 | **Buffer, and what only running finds.** Second live runs on the final code, the re-certification runs (laptop_price, tweet_irony, EuroSAT), README "What v0.7 adds", LIMITATIONS sync, version bump, launch drafts | release PR | |
| Sat Oct 10 | **The release gate** per the standing checklist | | |
| Sun Oct 11 | **Release v0.7.0**; comparison-table cost rows flip to shipped; the parked cost-win post angle becomes honest here | v0.7.0 out | |

**Cut order if a day runs short:** the `also:` line and the `prompts.yaml` block (Day 1); GCP through a user key (Day 2, the shipped file stands in); the researcher's budget sentence (Day 3); a second round of the back-and-forth (Day 4, one round ships). **Backlog behind the release:** carry-ins 2 to 12, quantization, a measured latency inside the run, tokens a second for local models on cloud GPUs, choosing between target models inside a prompt run.
...
### 2026-09-26 | Sprint 5 Day 3 | The serving budget is a wall

**Task:** Day 3 of v0.7 (the Sun Sep 27 row, built Saturday evening), from the plan gate Tony approved on Sep 26 with six calls: the one sentence lives in the over-budget block and not in the system prompt, so a run without a budget sends today's bytes; the post-run filter for prompts and tables ships today; with everything over budget the run may stop and say why, and the round-trip is Day 4; the live check is EuroSAT at $50 and 50,000 requests an hour, three iterations; the budget is dollars a month; and a budget typed mid-run re-prices from the next brief, `no budget` drops the wall, and only the user moves it. Days 1 and 2 priced the winner after the run. Today the price is a wall in three places, and nothing changes without `--serving-budget`.

**Before the loop.** The baseline itself is priced from what is in hand before a folder is made: the plain CNN at its size, the default boosting pipeline on the feature count, the starting prompt on the model under test with the task text as the estimate. A budget under that is refused like a wrong metric is, with the price, the budget and the cheapest machine in the table (left out for an API, which has no machine). `serving.baseline_facts` builds the facts, `Wall.price` prices them, and the refusal sits above `_ignore_run_folder` with the other refusals; the CLI tests check that no folder is left behind on all three families.

**Before each brief, on an image run.** `vision_levers.entry_recipe` turns every entry on the ready line into the recipe it would train: a backbone swap replaces the network whole (an own model or a from-zero stack does not travel, a head does), a bigger image keeps the incumbent and changes the size, an own-model entry names the timm network at its pretrained size, a stack entry is a `layers_net`, a head entry sits on the backbone the move names; epochs, augmentation, regularisation and the fine-tune depth return None, the incumbent's own network, which costs what the incumbent costs. `split_by_budget` prices each through the wall and keeps the line's own order; the entries over the budget go under a heading beneath the line, each with its monthly price, and the line reads "none; every open entry is over the serving budget" when nothing is left. An entry the table cannot price stays on the line: the wall acts on numbers it has. The price gate sits in `_vision_violation` after the no-class and no-value checks and BEFORE the not-ready check, because an entry the budget took off the line was opened by this run's numbers, and the nudge has to name the wall it hit rather than say the numbers never opened it. It names the first entry on the line that fits, top down, never the cheapest, Tony's call of Sep 25. A persisted over-budget brief falls back to that first entry; with nothing affordable left it is never accepted, the attempts run out into a proposer failure, and patience decides the run.

**After each experiment, on every family.** `_hold_against_the_wall` prices the finished experiment from what it did, through the same reading of an experiment the end of the run uses (the recipe an image session submitted, the estimator classes a table session used, the tokens a prompt spent per record), stamps `changes["over_budget"]` with the dollars, and the loop never banks it as the best, whatever it scored. The history row reads `[over the serving budget at $37 a month: a real score, but never the winner]`. The serving line gains `within the $50 budget,` or `over the $50 budget,`, written to best.json and prompts.yaml as well, and a run that refused anything ends by listing what it refused with the prices.

**The wall is one object.** `serving.Wall` holds the request rate, the price table, the budget and the reading of an experiment; the CLI builds it once, before the folder, and the Supervisor and the loop share it. So a typed `budget $80` moves both from the next brief on, `no budget` takes the wall down, a zero is refused with the shape to type, and nothing already recorded is re-judged. The typed line is read before the Supervisor routes it, so it is never a steer. `Priced(usd_per_month, fits, host)` is the answer everywhere, and None for the cost means the table could not price it, which is never a refusal.

**The probe, gemma4:12b at the run's own 0.4, ten calls a case.** History: the recorded EuroSAT first try from this machine's memory db (resnet18 at 128 px, 37 s an epoch, 0.9837), carried as the best. Its ready line: backbone to convnext_tiny (at 128 px, $49.06 a month at 50,000 requests an hour, four t4g.small) and image-size 224 (resnet18, $12.26, one box); epochs did not open. The plan's $50 did not separate the two, so the probe ran at $40. First answers with the wall: 10 of 10 briefed the affordable image-size entry, 0 of 10 named convnext_tiny. The control with no wall: also 10 of 10 image-size, so the first-answer count says nothing about the wall on this fixture; this 12B prefers the entry with a number from a fit line behind it, as PR E's probe found on Sep 21. The count that measures the wall: given a scripted over-budget brief first, the nudge brought back an affordable brief 10 of 10 times, every one copying the entry as written ("keep the best and train at 224 px", $12.26), and the harness fallback fired 0 of 10 times, in 9 s a reply. Seen on the way, and not Day 3's: 18 of the 20 unnudged first briefs wrote the value as a bare "224" with no unit, which the existing no-value guard sends back for "224 px"; the two that copied the entry whole priced on the first answer.

**The live check.** Two runs on this Mac, gemma4:12b driving, the code as committed. The approved one first: EuroSAT, `--serving-budget 50 --requests-per-hour 50000`, three iterations. Baseline 0.9496; the fallback brief carried every iteration, because the 12B stated each value as a bare number and the no-value guard sent it back twice (the same shape the probe saw); resnet18 at 64 px 0.9837, at 128 px 0.9785, then epochs on the 128 px best 0.9857, the winner. Its serving line: about $12.26 a month at 50,000 requests an hour on aws t4g.small, within the $50 budget, on the AWS list refreshed that day and GCP's shipped rows, with gcp e2-small at the same price beside it. The wall refused nothing, and the numbers say it could not have: with 37 s epochs the line never reached convnext_tiny at 224 px, and convnext_tiny at 128 px prices at $49.06, a hair under $50. That run printed the pre-review wording of the clause ("within the $50 budget"); the review renamed it "serving budget" while the run was going. The second run, `--serving-budget 20`, two iterations, was meant to show the wall taking convnext_tiny at 64 px ($36.79) off the line. It did not, and the log says why: the 12B's own first brief went straight to resnet18 at 128 px (0.9869, the winner, $12.26 within the $20 serving budget), and at 128 px this Mac's GPU runs about 400 s an epoch, so at the second brief the fit-time budget had closed every lever that costs more before the serving budget could see one. The line was empty, and what followed found a gap: the model briefed "own-model" twice with no loadable name, and the older "nothing ready, research" path accepted it, so the coder installed timm and trained a network the wall could not price (0.9617, not the winner). Off a line the wall emptied nothing is accepted now; a persisted brief runs out into a proposer failure and the run says why, until Day 4 sends the Researcher back. The approved pass criterion, convnext_tiny refused live, cannot be met on this machine: convnext never opens by time here. So a third run was set where the table separates on this machine: `--serving-budget 20 --requests-per-hour 400000`, two iterations, where resnet18 at 64 px is one small box ($12.26) and at 128 px two ($24.53). The wall acted live in both of its places. The 12B's first brief named resnet50; the price gate refused it at $25 against $20 and the nudge named the 64 px entry; the model's second brief was "backbone: resnet18" with no size, priced at the session size and let through, and the coder then trained it at 128 px, so the finished experiment was stamped $25 and never the winner (0.9856, better than anything the $50 run banked). The second brief fell back to the 64 px resnet18 entry after two no-value answers, and the coder trained convnext_tiny at 128 px instead, stamped at $368 and never the winner (0.9854). The run ended with no winner, both rows carrying their price in the summary table, and two lines naming what was refused and what was trained over the wall. That is the wall holding through a coder that does not follow the brief's network, which the pre-brief price cannot see and the post-run stamp exists for; LIMITATIONS says so now. The run also read "no candidate beat the baseline" when both had; the closing line now says no candidate within the serving budget beat it, and that the ones that did could not be served for the money. Before that run the gate also learned to read a size a backbone brief states, since "resnet18 at 128 px" trains at 128 px.

**What the build found.** With the real recorded history the plan's numbers were off: convnext_tiny at 64 px costs $36.79 a month at 50,000 requests an hour (three small boxes), not the $98 the read-out quoted for 224 px, and at 128 px $49.06, a hair under the $50 the live check runs at; the fixtures pin $20 and the probe $40. A brief for an over-budget network whose class had only that one entry used to fail the not-ready guard with "nothing in this run's numbers opens backbone yet", which is false, so the price gate moved ahead of it. A persisted over-budget brief with nothing affordable on the line was accepted by the "no lever ready, research" branch, which would have trained the network the wall refused; it now runs out into a proposer failure. Ollama on this Mac answered 500 to every call for twenty minutes: the brew install (0.30.5) has no runner binary, and the desktop app's own server (0.34.2) is what the runs use now.

**What review found, and what it moved.** Three reviewers again, one lens each: the wall in the loop and the gate, the prompt text and the docs against the code, the tests and CI safety. Every finding was reproduced before it was fixed. The ones that changed behaviour: the cheapest-machine floor in the refusal ignored memory fit, so on a machine with a refreshed AWS list it named a 0.5 GB box the baseline cannot run on and a budget that would be refused again (it now names the cheapest machine that could hold the baseline, and no machine for an API); the prompt baseline was priced at one input token, because the refusal passed zero characters where the wall had the real prompt (a 2,000-character prompt file now prices at its size before the run starts); the prompt model fell back to an empty name when it came from settings, un-pricing the winner on a plain default run; the wall was built below the memory archive, so `--fresh` with a refused budget archived the user's memory for a run that never started (the wall now sits above the archive with the other refusals); the `--spec` lane had no wall at all and could bank an over-budget estimator (the Orchestrator now shares the same wall and stamps the same way); a budget typed while a session trained re-judged that experiment under the new number against the reply's own promise (the loop keeps the budget each brief was judged under); an own-model best priced its entries as resnet18, since the incumbent came from `fit_recipe_of`, which is empty for own code (`recipe_of` now, the same recipe the ladder sizes from); an entry that keeps the best's network priced as nothing and fitted for ever, so a budget typed under the best trained work that could never be banked (it prices as the best's network, and the whole line closes); pricing errors in the supervisor and the pre-loop refusal were unguarded where the loop's were (`Wall` never raises now, and unpriced is never refused); with nothing affordable on the line the nudge asked the model to copy an entry that did not exist (a nudge of its own tells it to stop); and the refusal advised lowering the request rate when the budget was under the cheapest box, where no rate helps. The words moved too: "the budget" collided with the 540-second fit budget the same prompt already names, so every wall sentence says "serving budget"; "never briefed" was false for tables and prompts in the help and the README; two DECISIONS rows had lost a column, and one duplicated the Pricer row; the typed-budget replies spoke of "the line", which a table run does not have; budgets print as typed, so $36.80 never reads as the $37 beside it; and the end-of-run line now says how far each refused thing got, at the budget of the moment, whatever the budget is now. The tests moved off the shipped snapshot's dollar figures onto a fake table they own, and gained the first-iteration block, the end-of-run line, a wall whose budget nothing crosses sending main's bytes, and the spec lane's stamp.

**Not in this PR, by design:** the Researcher round-trip when everything is over budget (Day 4); pricing a table brief before it runs (a technique names no model) or a prompt brief (the tokens are known after); a budget in anything but dollars a month; a wall on a network the table cannot size. Not measured: a run whose ready line was ever entirely over budget.

**Tests:** 38 new, 2,355 passed here with the one pre-existing `.env` failure deselected; `ruff check src tests` clean, mypy strict clean on 78 source files. The gate tests price on a two-row table they own (one $7.30 box and measured milliseconds that put convnext_tiny at 64 px on two boxes and resnet18 at 128 px on one), so the next snapshot refresh moves none of their numbers; the CLI refusal tests compute the expected price from the shipped table at test time, and every test runs with its own cache folder. The pinned image digest holds with a wall that carries no budget.

### 2026-09-26 | Sprint 5 Day 2 | The Pricer prices against the clouds' own current lists

**Task:** Day 2 of v0.7, from the plan gate Tony approved on Sep 26 after his call that a static list of machines is baseless for a production tool. Day 1 priced every winner against a snapshot of 13 machines read once on Sep 25. Today the Pricer prices against the clouds' own current price lists, over every machine they sell in the region, and every line says which list it used and how old it is. The run itself is unchanged: no budget, no refusal, no lever, no prompt change. The wall is Day 3.

**Three clouds, three truths.** AWS publishes its price list per region as a file, and the us-east-1 file is 303 MB as CSV and 481 MB as JSON, refreshed daily. That cannot be fetched in a run, so `iterate prices refresh` streams it once through the `csv` module and reduces it on the way to on-demand, Linux, shared-tenancy, current-generation machines with their vCPUs, memory, GPU count, GPU memory and hourly price; the header is found by its first column rather than by counting metadata lines. Azure's retail price API is open, no key, 1,000 rows a page, and eastus is 9 pages and 8,770 rows in a few seconds; Spot, Low Priority and Windows rows are dropped and the cheapest meter per SKU kept. The API carries no vCPU or memory, so specs are read from the SKU name through Azure's naming convention and a family table, and a name the table cannot size is dropped and counted, never guessed: the D v1 and v2 names, whose number is a size code and not a vCPU count, are refused outright. Because the walk is small, `--cloud azure` also refreshes it in a daemon thread at the start of a run when the cache is a day old, silent on any error. GCP's catalog needs a key and prices per vCPU-hour and GB-hour, so its four shipped rows stand and the line says `gcp shipped 2026-09-25 (us-central1)`.

**The cache, the fallback, the line.** `core/prices.py` keeps one file per cloud and region under `~/.cache/iterate/prices/`, written atomically with a date and the URL. `load` prefers the cache per cloud and falls back to the shipped file, and the shipped fallback names the region it is for, so `--region eu-west-1` with no cache prints `aws shipped 2026-09-25 (us-east-1)` and a line saying how to get that region's prices. A refresh that reduces to nothing raises and leaves the old cache; an unreadable or future date reads as stale and never as a list; one list failing never costs the others. Offline with no cache, the shipped snapshot stands with its date, Tony's call. `iterate prices show` prints what the Pricer would read right now, every cached region included.

**Whole families, and the machines nobody measured.** The pick is a search over the machines a cloud sells: memory that fits the weights, capacity that covers the rate, cheapest month. A one-gigabyte floor keeps the half-gigabyte boxes out of a table winner; the weights plus a gigabyte keep them out of an image winner. Capacity on a machine nobody measured follows a rule said in the basis line: one request uses about two threads, so a box serves one worker per two vCPUs and half a worker on a single vCPU. GPU machines whose family the table knows price one box with no capacity claim unless the GPU is a T4, which keeps NVIDIA's published plain-PyTorch rate.

**The networks the agent names.** timm's published benchmark table (`benchmark-infer-fp32-nchw-pt240-cpu-i9_10940x-dynamo.csv`) is most timm models at batch size 1 with parameter count, multiply-adds and milliseconds per image on one CPU. A reduced copy ships (1,434 rows over 1,119 networks, 57 KB) and refreshes with the prices. 926 of the 1,101 names in `timm_models.txt` are in it, so the agent's own network is now priced when its name is there, as the stock network of that name at the image size the agent passed to `evaluate()`, and `facts_from_model_name` is ready for the Researcher's proposals on Day 3. timm measured with `torch.compile` and a plain `load().predict()` runs eager: convnext_tiny at 224 px is 14.5 ms in timm's table and 527 ms eager on this Mac, while resnet50 is 14.9 against 49.8, so timm's times are scaled relative to resnet50 measured here and the basis says a plain deployment can be slower.

**Measured on this Mac, two refreshes.** The first walk: AWS us-east-1 1,303 machines in 1 minute 48 seconds end to end, Azure eastus 948 machines with 729 dropped, timm 1,434 rows. After the review fixes below: AWS 1,289 machines in 57 seconds, Azure 929 sized and 748 dropped (constrained-vCPU sizes of known families, names outside the pattern, B v2 sizes, the D v1 and v2 names, the M, L, FX, HB, DS, A and ND families, the fractional A10 sizes). With the live list, resnet18 at 128 px lands on AWS's cheapest 2 GB box at $12.26 a month, the same as GCP's shipped e2-small; the 0.5 GB boxes are kept out by the memory rule; convnext_tiny at 224 px at 50,000 requests an hour scales to eight of those boxes at $98.11.

**What the build and integration found.** The shipped timm copy had one row with a blank field that crashed the loader; the copy is rebuilt without it and the loader skips a bad row. A test that cached a variable under the module's own name shadowed the module, a Python scoping trap, renamed. The snapshot integrity tests read the default two clouds and lost Azure; they read the whole shipped table now. Every test now runs with its own cache folder, since a refreshed list on the developer's machine would otherwise change what a priced winner prints.

**What review found, and what it moved.** Three reviewers again: the reducers and the cache, the wiring, and the docs against the code and the numbers. Sixteen code findings, all reproduced first and all fixed: Azure's D v1 and v2 names carry a size code and were read as vCPU counts, so D11_v2 priced as 11 vCPUs; a fraction of a GPU (`GPU` 0.125 on the g6f family) or an accelerator with no GPU count (Gaudi, FPGA, video cards) landed in the CPU pool; an AWS metadata line more than five would have shifted every field; a refresh that reduced to nothing was cached and then beat the shipped file for a day; cache writes were not atomic while a background thread and a refresh could both write the same file; a cache date that could not be parsed crashed the run on the main thread; the shipped fallback did not say which region it was for when another was asked; a 1-vCPU box got the full 2-thread rate; the tabular basis line halved the measured time on bigger boxes; the table winner's half-gigabyte floor let a 0.5 GB box through; a timm pretrained tag read as an unknown network; the F v6 family has twice the memory the code gave it; the Azure GPU rows had the wrong memory; the background thread caught three exception types and printed a traceback into the live TUI on a fourth; a unit test fired a real request to Azure; and the flag checks ran after the setup wizard. On the docs, eighteen findings, the sharpest that the coverage count was off by one (926, not 927), that the own-network row claimed to see a changed head the harness cannot see, and that the measured counts would change with the very fixes above, which is why this entry carries both walks.

**Not in this PR, by design:** the wall, `--serving-budget` and the price gate (Day 3); the Researcher and the Pricer going back and forth (Day 4); GCP through a key (designed, the first cut); a background refresh for AWS (its file); a GPU rate for anything but the T4; API model prices as a feed (none exists anywhere); a measured eager-versus-compiled gap per network family. Not measured: any cloud machine; the vCPU rule is a rule.

**Tests:** 65 new, 2,300 passed here with the one pre-existing `.env` failure deselected; `ruff check src tests` clean at both the CI and the Makefile scope; mypy strict clean on 78 source files; the price snapshot and timm's reduced table ship in the wheel the way `timm_models.txt` does. Every test runs with its own cache folder, so none reads the refreshed lists on this machine. The fetchers are covered against stub clients; the two real refreshes above are the live check.

### 2026-09-26 | Sprint 5 Day 1 | The winner comes with its serving price

**Task:** Day 1 of v0.7, from the plan gate Tony approved on the evening of Sep 25. Before today a run ended with `best: fine-tune resnet18 at 128 px (accuracy=0.9876, +0.0380 vs baseline)` and nothing said what it costs to run that model for real users. Now one more line follows it: what the winner costs a month to serve at the request rate, on the cheapest machine or API that can serve it, and where each number came from. The budget wall that refuses a candidate is Day 2; today is the number, and nothing the run does changes. No prompt moved, no lever moved, and no model sees a new line.

**The price snapshot.** `core/serving_prices.json` ships inside the package the way `timm_models.txt` does: 13 machines across AWS, GCP and Azure (two CPU boxes per cloud, a T4 box on each, and the L4 and A10 boxes where the cloud sells one) and the 12 API models a prompt run can name today on OpenAI, Together and DeepSeek. Every row carries its source and the day it was read, 2026-09-25, and every priced profile prints the snapshot date. The AWS and GCP rows come from secondary mirrors because the official pages are script-rendered; Azure's retail price API needs no key and answered directly. Groq retired its self-serve Llama models in August, so a Groq target is unpriced with the reason printed. DeepSeek renamed its models and prices them by the hour of day; the table stores peak and says so.

**The Pricer.** `core/serving.py` is arithmetic over that table and nothing else: it never reads an abstract, never asks a model and never imports torch, so it runs on the host after every family and cannot invent a price. A month is 730 hours. An API model costs tokens times price per request. A machine-hosted model takes the cheapest row whose memory fits the weights, with instances rounded up to cover the rate; a row with no benchmark rate cannot promise to cover it, so it is offered only when no rated row fits at all, and then prices one box and says the capacity is not estimated. `schemas/serving.py` holds the contract: `ServingFacts` in, `ServingProfile` out, a `basis` line for every number so an estimate is never read as a measurement, and a profile is either priced or says why it is not.

**The facts, per family.** A table winner never reaches the host, only its predictions do, so it is priced by the estimator classes on the last error-free cell that names one, up to the cell that wrote the predictions, and that family's single-row latency measured on the corpus. An image winner is priced by its recipe, which already reaches the host as the SUBMITTED line: the three pretrained backbones from torchvision's table with the ImageNet classifier subtracted, a `layers_net` or a `head` through the layer grammar's own `count_weights` and `count_macs`, `drop_stages` narrowing the head and said out loud. A prompt winner is priced by its model under test and the tokens it spent per record. `--requests-per-hour` (default 1,000) sets the rate; the line prints after `best:`, and the same block lands in `best.json` and, for a prompt run, in `prompts.yaml`.

**The kernel keeps the tokens.** The prompt runtime has counted tokens per record on every call since v0.5 and thrown the totals away. `submit()` in the prompt preamble now asks through a small inner helper and writes `tokens_in_per_record`, `tokens_out_per_record` and `records_measured` into `prompt.json` beside the answers fingerprint. Cached answers cost no tokens, so only the records really asked count; when every answer came from the cache, or a backend reported no usage, the winner's own prompt text stands in and the basis line says so. The printed `ask:` line is byte-identical to before, pinned by a test.

**Measured on this Mac, two threads, batch 1, torch 2.14.** resnet18 6.9, 11.1 and 26.3 ms at 64, 128 and 224 px; resnet50 13.2, 21.7 and 49.8; convnext_tiny 166, 265 and 527. Fixed overhead dominates small images: resnet18 at 64 px takes three times what scaling its own 224 px time by multiply-adds would give, which is why the CPU rate is a measured table per size and a custom stack scales from resnet18's rate at the nearest size. An independent measurement on an AWS c7i.xlarge (PyTorch 2.8, FP32) puts resnet50 at 224 px at about 44 ms, within 12% of the 49.8 here, so these numbers stand in for a 2-vCPU box until one is measured, and every profile that uses them says so. For tables, the slowest single-row predict per estimator family over laptop_price, mobile_price, heart_risk and adult_income on scikit-learn 1.8.0: linear 2.9 ms, tree ensemble 12.2, boosting 4.5, nearest neighbours 3.2, svm 2.9, mlp 2.8, doubled as the margin. The one apples-to-apples GPU rate is NVIDIA's own plain-PyTorch batch-1 ResNet-50 number for the T4, 10.7 ms; the L4 and A10 numbers in MLPerf are INT8 TensorRT, twenty to thirty times faster than the torch file a run delivers, so those rows carry no rate.

**What the build and integration found.** The estimator's first draft priced a machine by capacity at any rate, and a machine with no rate at one box; the no-rate rule above came from watching an L4 "win" at two million requests an hour. The `render()` first excluded the chosen row from the `also:` line by object identity, which held until the profile went through JSON and back. A local Ollama model with no size in its name priced as if it weighed nothing; it is unpriced now. The pre-existing `test_a_run_that_is_refused_leaves_no_run_folder_behind` fails on this checkout and passes in a clean worktree: the settings loader reads the `.env` in the working folder, which carries an e2b key, and the test never isolates it. It is the v0.6.1 item with its cause corrected, and it is on Day 6.

**What review found, and what it moved.** Three reviewers, on arithmetic, wiring and the docs against the code. Ten findings on the code, all reproduced first and all fixed: the table pricer could price a cell the agent ran after it had already written its predictions, since sessions explore after submitting, so the cell list now stops at the last cell that wrote `predictions.csv`; `Quantile`, `KNeighbors` and `SGD` as prefixes swallowed `QuantileTransformer`, `KNeighborsTransformer` and `SGDOneClassSVM`, so the prefixes are whole class names now; `drop_stages` was ignored, and one dropped stage is most of a backbone's weights, so the head width comes from the kept stage and the basis says the count is for the whole backbone; a stack with nothing to compute divided by zero; a backend that reports no usage priced a prompt at zero dollars as a measurement; the cached-answer estimate read the baseline prompt instead of the winner's; `mixtral:8x7b` read as 7B; `--backend openai-compatible` aimed at OpenAI's own endpoint never priced, so the provider is now the cloud the base URL points at through the factory's alias map; the family order put mlp above linear against the measured table; and torchvision's weight counts carry the 1,000-class classifier the served network replaces. On the docs, 22 findings: every number reproduced except the "1.8 ms, four times" claim, which was resnet50's scaling and not resnet18's own (2.1 ms, three times, corrected in both files), and a handful of claims that said more than the code did ("every profile prints the snapshot date", "the last working cell", the T4 number's "eager"), corrected to what the code does.

**Not in this PR, by design:** the budget wall and `--serving-budget`, the ready-line filter, the price gate and the memory of refusals (Day 2); live prices fetched at run time with a cache, whole instance families, `--cloud` and `--region`, and the sizes of the networks the Researcher names from timm's results table (Day 2, pulled forward from a Day 6 refresh script when Tony said a static list is not where this can stay); the Researcher and the Pricer going back and forth until a model fits (Day 4); `iterate cost` and filling `cost_usd` (Day 5); a measured inference latency inside the run (backlog); tokens a second for local LLMs on cloud GPUs (backlog); the wiring test that drives a prompt winner through the CLI into `prompts.yaml` (the writer is tested, the CLI path for prompts is not). Not measured: any live run, since Day 1 has no LLM in it; any cloud machine.

**Tests:** 73 new, 2,235 in the suite here with the one pre-existing `.env` failure deselected; `ruff check src tests` clean at both the CI and the Makefile scope; mypy strict clean on 77 source files; the price snapshot ships in the wheel.
...
**Honest state of the evidence:** the claim now rests on live runs of the FINAL code, not of a branch. See the release-day entry for the four of them. What follows is what was true before the hold. EuroSAT on the Day 6 branch: `f1_macro` 0.9477 to 0.9848, 3 iterations, 43 min. Flowers102 on main 6485be3: accuracy 0.5544 to 0.9670 against a 0.9743 ceiling, 98.3% of headroom, about 29 min. Storm wind on main 6485be3, the machine under memory pressure: rmse 13.12 to 9.1617 against 8.87, about 93%, 48.5 min. After the keep-best guard, Flowers102 on branch commit 3f6c8cb: accuracy 0.5544 to 0.9811, 39 min, which is 101.6% of the stored headroom, because convnext_tiny at 224 px is a pairing the ceiling sweep never ran; the gap over the ceiling is 0.68 points, under two standard errors. On the final code: EuroSAT on the released code, 2026-09-21: the baseline reproduced the stored 0.9496 exactly and iteration 1 reached **0.9876** (resnet18, 128 px, 10 epochs), above the 0.9848 this release was certified at; stopped on patience after 4, no traceback, and `load(best_model.pt).predict()` re-scored 0.9876 over all 5,400 sealed images. On CUDA: the baseline and the out-of-memory capture ran on an RTX 4050, a whole agent run has not. Most of each gain is pretrained features. The 12B wrote torch of its own in one of eleven sessions, and that model lost to `fit()`, 0.9478 against 0.9848. Across releases, the trajectory bar on laptop_price: 55.4% of headroom at v0.4, 56.2% at v0.5, 73.3% at v0.6, one run each, so a pass and not a measured improvement. **What slipped:** v0.5's posts and release notes promised cheaper prompt sampling in v0.6, "a quarter of today's calls". It is carry-in 1, and it moved to v0.7 on 2026-09-16 with the other eleven when Tony put the release back on Sun Sep 20; the v0.6 posts and notes say so. The cloud-GPU interface was cut to post-v1.0. The eval suite's version-over-version table is still nearly empty (carry-in 11). **The calendar:** first planned for Aug 16, re-dated on Sep 12 after v0.5 waited four weeks on its certification run. The data linker added three build days and moved the release one Sunday on Sep 14, and on Sep 16 Tony put it back on Sun Sep 20 by moving the twelve carry-ins to v0.7. Day 7 did not start on Saturday, so its build, the gate runs and the release shared one day.
...
**What a fit takes now.** `layers=[...]` builds a whole network from zero, backbone `layers_net`. `head=[...]` puts your own layers where a pretrained backbone's single final layer was. `drop_stages=N` takes the last N stages off first. `unfreeze="last_block"` trains only the last stage that is left, plus the head. A stage is a large group of pretrained blocks, not a layer, so a fit that drops one says so out loud: `drop_stages=1 removed layer4 from resnet50, with their pretrained weights; the head now sees 1024 numbers per image instead of 2048`. `net.STAGES` names each backbone's four stages and the width each one ends on, read from torchvision 0.26 and checked again on 0.29; `model_for` branches layers first, then head or drop_stages, and otherwise takes the stock build it always took. Before it cuts anything it checks that every name in the table is really in the model and that the width the table records is the width the stock head reads, and a torchvision that moved either gets a refusal naming its own version.

**The stock branch is bit-identical, proved before anything else was written.** One script run against a `git archive` of main and against the branch, in separate processes: ten stock builds (every backbone, every unfreeze, 3 outputs and 1) hashed weight by weight under one seed, the probe-head copy hashed the same way, and a whole 4-epoch plain-CNN fit on the CPU. Every digest equal, the four printed loss lines equal to the last digit (`loss=0.3041 train_acc=0.8646`, then 0.0592, 0.0260, 0.0261), the output digest `17ae94c6...` the same on both. That is R2 check 5.

**simple_cnn is the stack, and the stack is simple_cnn.** `build_scratch(conv(32) pool conv(64) pool conv(128) pool)` and `_simple_cnn` give the same state_dict keys in the same order and the same weights under one seed, and the same module types in the same order. The baseline is NOT routed through the builder, so no stored number can move; this check is what says the two agree. Real parameter counts match the pure-Python counter exactly on three stacks at 1 and 10 outputs: simple_cnn 93,825 and 94,986, the test stack 19,649 and 20,234, a strided one 10,305 and 11,466.

**The three caps, measured (R2 checks 1 and 2).** The Mac had other work on it: system swap was already 11.9 GB used before check 1 and moved in both directions during it, so swap is not a clean signal here and RSS and the MPS allocator are.

The heaviest spec the caps allow is `conv(48) pool conv(256) pool conv(512) pool conv(512) pool conv(512)` at 224 px, batch 64: 7.46 billion multiply-adds an image, 6.02M weights, 5.98 GB of activations. Its 11 untimed planning steps cost **12.43 s** on MPS, 1.12 s a step. The read-out said the cap comes down if that passes about 60 s. It does not, so MAX_MACS stays at 8e9 an image. What the caps turn away, each by the right one: `conv(96) pool conv(192) pool conv(384) pool conv(512)` at 224/64 on activations (6.8 GB); the same allowed spec at batch 128 on activations (9.3 GB), which is the batch-size hole the skeptic found; `conv(128) pool conv(256) pool conv(512) pool conv(512)` at 224/64 on multiply-adds (9 billion); and the heaviest spec again at 384 px on multiply-adds (21 billion).

A spec just under the weight cap, `conv(64) pool conv(256) pool conv(512) pool conv(512,7) pool conv(512,7)` at 96 px, counts 27,029,642 weights against the 30M cap. Twelve AdamW steps at batch 64, 22 optimiser states: system swap did not move at all (5895.81 MB before and after), process RSS went 685.5 to 721.9 MB, the MPS allocator held 0.41 GB with 2.19 GB reserved by the driver. gemma4:12b was not loaded beside it, because no Ollama run was started for this check; the 8.36 GB it holds is the existing LIMITATIONS row, and the headroom above is what is left for it.

**Out of memory on a layers net says what to do (R2 check 3).** Forced with the MPS watermark at 0.02 on `conv(64) pool conv(128) pool conv(256) pool conv(512)` at 224 px, batch 64. One clean `DeviceOutOfMemoryError`, no traceback: `out of memory on mps at batch_size=64, image_size=224: halve one of them, or put a pool after the first conv`. The repair lever's own regex still reads 64 and 224 out of it, which is why the `batch_size=.., image_size=..` prefix is kept ahead of the new hint. The `buffer` kind gets the same hint in its own words.

**One A/B so a claim has a number behind it (R2 check 4).** EuroSAT, 960 training rows and 240 holdout at 64 px, one fit each, same fold and same seed, BUILD_LOG only: no stored ceiling moves and no sweep rung was added.

| Recipe | Holdout accuracy |
|---|---|
| resnet50, head only, 4 epochs | 0.8083 |
| the same plus `head=linear(512) dropout(0.5)` | **0.8750** |
| simple_cnn, 20 epochs from zero | 0.8000 |
| `layers=conv(32) pool conv(64) pool conv(128) pool conv(256) pool`, 20 epochs | **0.8542** |

A head is worth 6.7 points on this slice and one more conv block is worth 5.4. One slice, one seed, one fit each: it is enough to say the feature does something and not enough to move a ceiling.

**What the build changed from the read-out.** `printed(recipe)` is new: the FIT and SUBMITTED lines write a stack as the one canonical text, never a list, and leave a field out when the recipe does not use it. Without it every old run's line would have gained three keys, and the lever gate matches SUBMITTED to FIT by equality. The same function writes the `from` block, recipe.json, the preamble's `recipe now:` line and meta.json's baseline block, so a run that touches none of the new fields prints the bytes it always printed. `_train_mode` now puts in eval any batch-norm layer with no trainable weight of its own, instead of every batch norm under `unfreeze="head"`; for head and all that is the same result, and it is what lets last_block leave the stage it trains alone. A layers net drops a final training batch of one row, because batch norm in train mode cannot take one; stored recipes keep their batch order because only a layers net drops it. `describe()` and the sweep's `label()` learned the new fields, which is part 1 work the branch did not have.

**Guards proven load-bearing.** The round-trip pin from PR A passed by extension, never by weakening: `ROUND_TRIP_SHAPES` gained a network from zero, a head, a head with a dropped stage, and last_block, and all 17 shapes save and open again with a gap of 0.0 and nothing downloaded. Golden state_dict keys are pinned for one from-zero spec and one head spec, so a builder change fails loudly and forces a `SAVED_FORMAT` bump. `drop_stages` widths checked on every backbone with `pretrained=False`: resnet18 512/256/128, resnet50 2048/1024/512, convnext_tiny 768/384/192, each equal to the table and each giving the right output shape. The last_block trainable set checked on every backbone at 0 and 1 dropped stages, with the frozen stages' batch norms still in eval. A head of dropout alone still ends in the probe's own weights.

**What the review changed.** Six fixes, each with a check that is red without it. Two places still wrote the recipe with `asdict`, so an image session opened by advertising `layers`, `head` and `drop_stages` to the coder, and meta.json's baseline block carried them too: both now go through `printed()`, which is what the paragraph above promised. `fit(epochs=0)` is the other spelling of the probe, and after a `head=` or `drop_stages=` fit it was refused, with a refusal that earned a second one; `_tops` now reads the epochs rule `merge` applies, so both spellings drop the carried head the same way. `checked_meta` checks the three new fields as well, so a hand-edited file with `drop_stages: 99` is refused in plain words instead of dying on an index error. The torch child named for the one-row-batch skip did not exercise it: its stack ended on an 8 px map, where batch norm takes a batch of one; it now runs four stride-2 convs down to 1 px, and deleting the skip turns it red. The refusal for a torchvision that moved a stage said to install 0.24 to 0.26, below what pip installs today, and now says 0.24 or newer and asks for the version; the table is checked on torchvision 0.29. `MAX_BATCH_MACS` was the per-batch cap from before the cap went per image, read by nothing, and is gone.

**Tests:** 2057 in the suite, 171.3 s and 198.4 s on two runs with other work on this Mac: 207 more than 25bd892, the commit this branch sits on, and 62 more than part 1. `ruff check src tests evals` clean, `ruff format --check` clean on all 15 changed `.py` files, mypy strict clean on 89 source files. Seven new torch children and one new real-kernel check ran on this Mac; all of them skip on CI with a reason. The one pinned test that moved is the one the read-out allows, `meta["backbones"]` in test_dl_target.py, which gains `layers_net`. Two non-pinned tests moved with the printed-line rule: `test_the_fit_line_carries_...` compared the line's keys against `asdict(recipe)` and now compares against `printed(recipe)`, and the meta test compares the baseline block the same way, which is what made the stored bytes move in silence.

**Not in this PR, by design:** the two lever classes, the ready line, the user ask (PR D) and every prompt key (PR E), so no image run can reach these settings through the supervisor yet. No sweep rung and no METHOD bump. Not measured: a layers net on CUDA, a live 12B run that briefs one, and whether a 12B writes a stack that parses, which is probe R3 on Day 14.

### 2026-09-20 | Sprint 4 Day 10 | Run All works on best.ipynb, for every family

**Task:** PR B of the v0.6.0 hold, the plan's Day 10, built the night before from the read-out Tony approved. `best.ipynb` opens with the session's own first cell, and that cell reads `meta.json`, `train.csv` and `holdout.csv` from its own folder. The folder it read them from was the kernel's temp folder, deleted when the session ended, so Run All has failed at cell 1 since v0.2, for table and prompt runs as well as image ones. The deliverable this project calls "a runnable notebook" did not run.

**The fix, in one line each.** The bytes the kernel was started with are written beside the notebook. They are built once, from the same dataset object the loop hands the coder, so a notebook cannot load different rows than the run measured. The holdout copy has no labels in it, because the kernel never got them, and a test asserts that for all three families. A cell that errored in the session carries the `raises-exception` tag, so Jupyter carries on past it. An image notebook gains one host-written setup cell that puts the folder back the way the session found it.

**Why the setup cell exists.** An image session keeps state on disk. `.vision-session.json` is what the last fit was, so a second Run All would start from the first Run All's fit rather than from the recipe the session started with. `predictions.csv`, `probabilities.csv` and `recipe.json` are the submission, and the keep-best guard reads them: a second Run All would be held against the first one's score and print KEPT instead of SUBMITTED. The setup cell writes `incumbent.json` with the recipe the session departed from, taken from the experiment the loop recorded, and removes those four files plus a stale `.iterate-network.pt`. `best_model.pt` is not in that list and is never touched: it is read-only, and it is the network that was scored. Run All trains again and writes its own `.iterate-network.pt` in the run folder.

**Proved against real recorded sessions, not a fixture.** 98 notebooks were rendered from four run memories on this machine (a table certification run, the laptop-price run, the irony prompt run and Friday's image run), once on main 25bd892 and once on this branch. 52 came out byte for byte identical. The other 46 differ in exactly one thing: `"metadata": {}` became `"metadata": {"tags": ["raises-exception"]}` on the cells that errored. Nothing else in any file moved. One thing the comparison found on the way: nbformat gives every cell a fresh random id on every render, so two renders of one session already differ there today, and the pinned test normalises ids the way the comparison did.

**Measured.** On adult.clean.csv, 21,523 train rows and 5,381 holdout rows: building the three files costs 0.034 s and writing them 0.0004 s, and they add 2.65 MB to the run folder (train.csv 2.13 MB, holdout.csv 0.52 MB, meta.json 241 bytes). That is the user's own rows a second time, so `.iterate/.gitignore` holding `*` is written when the folder is first made, below every refusal, and a `.gitignore` that already exists is left alone. An image run's copies are small whatever the dataset: the csvs hold paths into `~/.cache/iterate/images/<key>`, where the run read its pixels.

**Run All, twice, on a real kernel.** A recorded session with a dead end in it, delivered into a fresh folder that already held a read-only `best_model.pt`. Pass one ran to the last cell, printed SUBMITTED once, printed no KEPT, and left `best_model.pt` byte for byte as it was. Pass two, over the folder pass one had written into, did the same: SUBMITTED once, no KEPT, `best_model.pt` unchanged, `incumbent.json` back to the recipe the session started from. The same notebook with the tag taken off stops at the dead end, which is what every Run All did before this PR. It runs in a kernel that is this interpreter, because the installed `python3` kernelspec on a Mac belongs to the system python.

**What moved and what did not.** `build_session_notebook` gained one keyword, `setup`, and with it unset the output is what main writes apart from the tag. Only `best.ipynb` gets input files: the per-iteration notebooks under `notebooks/` are the journey, kept for reading, and a copy of train.csv per iteration would multiply the data by the iteration count. Spec and one-shot notebooks now carry an absolute data path, since they load from where the run started and are opened from the run folder. `changes["started_from"]` is stamped inside the existing image branch of the loop, so table and prompt experiments gain no key. No printed line, no prompt byte, no lever and no stored number moved.

**What the review round changed.** Six holes, all in this PR's own surface. A Run All with `~/.cache/iterate/images` deleted trained on all-black frames and printed a score: `decode` turns a path it cannot open into a blank frame and says nothing, so the preamble now counts the paths that are not there, names the first one, and refuses to start when every one is gone. The `.gitignore` marker decided ownership with "or the runs dir already exists", which is true from the second run on, so `ITERATE_RUNS_DIR` pointing inside a project put a `*` in the project's root on run 2; ownership is now "this call made the folder, or it is called `.iterate`", and the guard test invokes the CLI twice with a run folder in between, which is the only way to catch it. The marker call moved below the e2b key check, the one refusal that can fire after the data is loaded, so a refused run leaves nothing behind again. The setup cell was going into every image notebook under `notebooks/`, where running one would clear the run folder; it is now gated on an explicit `with_setup`, not on `is_best`, because the winner also gets a journey copy. The prompt family got the CLI test the three-family claim needs: a `--loop-holdout` run's delivered rows are the loop's slice, not a second read. Two more rows in LIMITATIONS say what a prompt Run All costs (one live call per record unless `.iterate/prompt-answers.db` travels with the folder) and that a Run All leaves its staged fits in `.fits/`.

**Tests:** 16 more than PR A, 1866 in the suite, 144.1 s. `ruff check src tests evals` clean; `ruff format --check` clean on every `.py` this PR added lines to except `deliver/notebook.py`, `test_agent_loop.py` and `test_notebook.py`, which are left exactly as unformatted as they were on 25bd892, because CI runs `ruff check src tests` only. mypy strict clean on 88 source files. With torch hidden behind an import hook the eight touched test files give 425 passed and 15 skipped, every skip with "torch is not installed", which is CI's shape; the Run All check also skips where nbclient is absent. No pinned or byte-identical test moved.

### 2026-09-20 | Sprint 4 Day 9 | The winning network lands in the run folder, and three lines load it

**Task:** PR A of the v0.6.0 hold. Tony held the release on Sun Sep 20 because an image run trained a network and threw it away: "The user has to run the notebook to get the model he is going to plug in his production app." This is the plan's Day 9 (Mon Sep 21), built the night before, from the read-out Tony approved. Three designs were written and a skeptic attacked each; every hole the read-out marks real for this PR is built. Two builders worked one branch: the kernel side first, then the host side, the loader and these docs.

**Three hops.** The network used to die when a fit ended, and the kernel's folder is deleted before the loop knows which try won. One: every trained `fit()` stages its weights in a hidden `.fits` folder inside the kernel, capped at 1 GB with the oldest fit that is not the best dropped first, read by the metric's direction. When `submit()` lets a fit through the keep-best guard, the session writes `.iterate-network.pt` beside the predictions and records its sha256 in recipe.json. Two: before `close()` deletes the folder, the coder copies the file out to a host slot, and only when its digest is the recorded one. When the loop calls a try the new best, `settle()` moves it to `.iterate/runs/<run_id>/best_model.pt` and makes it read-only. Three: `iterate.vision.load(path).predict([...])` rebuilds the network from the recipe in the file and resizes images the way training did. The saved network is the one that was scored, trained on the 80% fold, which is the promise table runs make.

**The write order, and the names.** `_write` saves only after the KEPT return: the network to `.iterate-network.pt.part`, then the probabilities and predictions as before, then the rename, then recipe.json last. Nothing is deleted up front, so a Ctrl-C or a cell timeout part way leaves the earlier network, predictions and recipe.json all matching. `torch.save` reports a full disk as RuntimeError and not OSError, so staging catches both, removes the partial file and prints one fact; a fit never loses its predictions to its weights file. The kernel-side name is dotted and harness-only because `model.pt` and `best_model.pt` are the two names a 12B writes in its own torch code. The printed FIT and SUBMITTED lines gained no key, because the lever gate matches them by equality, and no line is printed when a network is saved, because every printed line is read by the agent.

**One builder, and a light module.** `targets/net.py` imports numpy, PIL and torch and nothing of the agent. The builder, the resize, the normalise step and the predict loop moved there with their bodies unchanged, and `dl.py` re-imports them, so `dl._build` and the rest are the same objects and every existing monkeypatch still works. `model_for(torch, recipe, outputs, probe_head, *, pretrained=True)` is now the only way a network is made: a fit, a save and a load all pass the whole recipe through it, so PR C's layer params extend one function and a saved file always rebuilds. A pin test lists every recipe shape `fit()` accepts and fails when a backbone, an unfreeze value or a Recipe field is not covered. `FitJob.save_to` defaults to None, so the plain-CNN baseline, the eval sweep and every stored ceiling never save and stay bit-identical; a fit with `save_to` was measured to give bit-identical outputs to one without.

**The loader.** `load()` refuses a torch older than 2.6, where a file opened with `weights_only=True` could still run code, always passes `weights_only=True` with no fallback, and checks every metadata field before it builds anything: format, backbone in the known set, task, 32 to 384 px, the class count against the outputs, a finite label centre and spread for numbers, the mean and std against this version's, and that the weights are a table of tensors. It builds with `pretrained=False`, so nothing is downloaded, maps to the CPU first and then picks a device. torch's own refusal of a pickled file tells the reader to pass `weights_only=False`; ours replaces it and says the file is not a network an iterate image run saved. A missing file is still a FileNotFoundError. `predict` takes paths, PIL images or arrays and answers in class names of the label's own type, or in the label's own units for numbers; one path in place of a list is refused with the fix.

**The host side.** `keep_model` is a keyword the CLI passes for the image family only. With it unset, which is every table and prompt run, the coder never asks the kernel for a network; a test runs both families through the real CLI and asserts their coder kwargs hold no `keep_model` and that no staging folder is made. The slot is cleared when a session starts. In `on_experiment` the settle runs first and in its own try, because the loop swallows a hook's error and the next session clears the slot; a failure is a warning with the path, and the staging folder is removed in a try/finally. At the end of the run the image default is `run_dir/best_model.pt`, `--output` moves it with best.json beside it as tables do, best.json gains `recipe`, and a run that saved no network says why in one line: no try beat the baseline, or the winner is the agent's own network, which v0.6.0 does not save, or the winning try left no file.

**Measured, M-series Mac, 10 classes.** Sizes are the delivered file in decimal MB. Times are one run each, in seconds.

| | plain CNN | resnet18 | resnet50 | convnext_tiny |
|---|---|---|---|---|
| saved file | 0.39 MB | 44.8 MB | 94.4 MB | 111.4 MB |
| stage a fit's weights in the kernel | under 0.01 | 0.04 | 0.09 | 0.06 |
| write the one file at submit | | 0.05 | 0.09 | 0.07 |
| sha256 of the file | | 0.02 | 0.03 | 0.04 |
| host copy-out (read, hash, write, rename) | 0.002 | 0.03 | 0.07 | 0.06 |
| settle into the run folder | under 0.001 | 0.02 | 0.05 | 0.07 |
| digest check at delivery | under 0.001 | 0.015 | 0.03 | 0.04 |
| `load()` on CPU | 0.004 | 0.09 | 0.19 | 0.23 |
| `load()` on the Mac's GPU | 0.015 | 0.14 | 0.29 | 0.31 |
| `predict` 64 images at 64 px, CPU | 0.04 | 0.20 | 1.22 | 0.74 |
| `predict` 64 images at 64 px, GPU | 0.14 | 0.04 | 0.06 | 0.05 |

So the whole save path costs a convnext_tiny winner about 0.3 s against fits of minutes. The 1 GB staging cap holds about 9 convnext_tiny fits or 23 resnet18 fits. Round-trip gaps against what the session wrote: a trained fit 5.6e-17 on probabilities.csv, the real plain CNN 0.0, regression 0.0, and a probe 3.9e-08 with every class the same, because a probe's predictions come from a float64 scikit-learn head and the file carries it as float32. Thirteen recipe shapes round-trip through the real builders with `pretrained=False`, max gap 0.0, with TORCH_HOME left empty. Through the real confined kernel: the cell staged `.fits/1.pt` and wrote the network inside the sandbox, the coder copied it out, the kernel's folder was gone, the digest matched recipe.json, and `load()` on the Mac's GPU predicted all 8 holdout rows as predictions.csv has them, with a probability gap of 0.0. A process that calls `load()` and predicts holds no pandas, scikit-learn, lightgbm or scipy, none of `iterate.core`, and not `iterate.targets.dl`. A load with every socket connect refused dialled nothing and downloaded nothing.

**What the build found.** The host side had one hole of its own. `_place` copies to a second name and renames, so a settle that fails part way leaves the earlier best whole. That is right for the file and wrong for the run: the earlier best's network stayed in the run folder, the end of the run found a file there, and delivered it as the winner's with a load hint and an `artifact_path`. Reproduced with a test before it was touched. `deliver()` now checks the file against the `model_sha256` in the winning try's own recipe.json and removes one that does not match, and a winner whose recipe.json names no network can never claim a file; the run then says the winning try left no network file. A try that carries no recipe.json at all is delivered as before. `_place` also makes a read-only best writable before it replaces it, because Windows refuses to replace a read-only file. The kernel side moved off the read-out in small ways: `net.py` also holds the backbone table, `RecipeError` and the size range, since the builder and the metadata check need them and `net.py` must not import `dl.py`; `checked_meta` refuses a file whose mean or std is not this version's, since the loader normalises with its own; `saved_meta` takes plain values and reads torchvision's version from package metadata, so it is tested on CI with no torch; the loader does not set the MPS watermark, which is for training inside the kernel and not for a user's app. `TorchRunner._embed` still calls `_build` directly, because it builds a featuriser from a backbone name and nothing it builds is saved; PR C decides whether `drop_stages` changes a probe's features.

**Each guard proven load-bearing.** Twenty deliberate breaks, one at a time, each caught by a named test and each file restored byte for byte. Kernel side, 7: the old file unlinked up front, staging catching OSError only, the cap ignoring the metric's direction, the `.part` kept on an interrupt, SUBMITTED gaining a key, the resize offset changed, the save moved before the KEPT return. Host side and loader, 13: the copy-out without the digest check, the slot not cleared at session start, settle's error escaping the hook, the settle moved after the report copy, the staging folder not removed, a table run handed a slot, the delivered file left writable, the delivery digest check removed, `weights_only` off, the torch version never checked, the metadata never checked, the build with pretrained weights, and the loader importing `dl.py`.

**Not in this PR, by design:** saving the agent's own torch network. The name an agent passes as `model=` is not the architecture it built, and an `hf_hub:` name makes a network call at load time, so an own-model winner saves nothing in v0.6.0 and best.json, the end-of-run line and LIMITATIONS say so; v0.7. safetensors with a JSON side file, which is not installed and would be a new dependency days before a release. A rule that `_kept_val` returns None when the network is missing: with the new write order a crash already shows as a predictions digest mismatch, and the rule would switch keep-best off after an interrupt. Run All on best.ipynb is PR B, the layer params are PR C, and no lever or prompt moved. Not measured: a live 12B image run with the saved network, which waits until PRs C, D and E are on one branch; `load()` on CUDA; a save onto a disk that is really full rather than a torch that says so.

**Tests:** 136 more than main, 1839 in the suite, 148.3 s. `ruff check src tests evals` clean, `ruff format --check` clean on every changed `.py`, mypy strict clean on 88 source files. The torch children and the real-kernel checks all ran on this Mac. With torch hidden behind an import hook, the loader and kernel files give 19 passed and 18 skipped, every skip with "torch is not installed", which is CI's shape. No pinned or byte-identical test moved.
...
**Tests:** 26 more than Day 6, 1,682 in the suite across 71 files; `ruff check src tests evals` clean; mypy strict clean on 89 source files. CI on main e843ab6 passed 1,648 and skipped 34, the torch-only and Mac-only tests. #76 then added 21: 1,703 in the suite, and CI passed 1,669 and skipped the same 34.
...
**Tests:** 207 more than main, 1656 in the suite; `ruff check src tests evals` clean, `ruff format --check` clean on every changed and untracked `.py`; mypy strict clean on 89 source files. Every image test runs on a fake runner and real PNGs, so the 283 tests across the files this PR and its review touched pass with torch, torchvision and timm blocked from importing, which is CI's shape.
