...
| Wed Sep 16 | **The monitor, PR 3 of 3.** Deterministic checks before iterating: coverage both ways, duplicate and conflicting labels, two-source agreement, class floors, group-leak columns, near-duplicates; a typed report at the pause, in the run dir and in the supervisor's data summary. Reports only, never edits. The label audit and the vision spot check follow the target | `adapters/data/monitor.py`, tests | |
...
**Tests:** 49 new, 960 in the suite; ruff clean at CI scope; mypy strict clean.
