...
**Tests:** 52 more than main, 2047 in the suite, 162.6 s. `ruff check src tests evals` clean, `ruff format --check` clean on all 13 changed `.py` files, mypy strict clean on 89 source files. Seven new torch children and one new real-kernel check ran on this Mac; all of them skip on CI with a reason. The one pinned test that moved is the one the read-out allows, `meta["backbones"]` in test_dl_target.py, which gains `layers_net`. One non-pinned test moved with the printed-line rule: `test_the_fit_line_carries_...` compared the line's keys against `asdict(recipe)` and now compares against `printed(recipe)`.
