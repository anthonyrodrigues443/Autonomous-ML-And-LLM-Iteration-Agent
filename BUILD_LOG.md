...
| Tue Sep 15 | **The Linker, PR 2 of 3.** The sixth specialist: when the ladder finds no plan or two plans tie, a capped listing of the folder goes to the model and it answers one tool call, table, key, target and task picked by index; the harness rebuilds and re-validates the frame exactly as a rules plan; the pause becomes a conversation, yes, no, or a correction in plain English, five rounds at most, `--yes` in scripts; the accepted plan is cached so a folder never goes through the Linker twice | `core/linker.py`, prompts.yaml keys, the pause loop, tests, a live pass on gemma4:12b | |
