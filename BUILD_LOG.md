...
## Sprint 5: v0.7, cost-constrained serving. Fri 2026-09-25 to Sat 2026-10-03, release Sun 2026-10-04

*(Re-dated 2026-09-25 at the Day 1 plan gate, four days after v0.6.0 shipped. The August dates below it were set before v0.5 waited on its certification run. A row is a calendar day and a day can carry several PRs. Tony's calls at the gate: the Claude API adapter stays in v1.0 with the benchmark; quantization moves to the backlog, since no full agent run has finished on CUDA; the price gate names the first entry on the ready line that fits, never the cheapest.)*
...
**Not in this PR, by design:** the budget wall and `--serving-budget`, the ready-line filter, the price gate and the memory of refusals (Day 2); `iterate cost` and filling `cost_usd` (Day 3); a measured inference latency inside the run (backlog); the price refresh script (Day 6 if green); tokens a second for local LLMs on cloud GPUs (backlog); sizes for the agent's own timm networks (backlog); the wiring test that drives a prompt winner through the CLI into `prompts.yaml` (the writer is tested, the CLI path for prompts is not). Not measured: any live run, since Day 1 has no LLM in it; any cloud machine.
