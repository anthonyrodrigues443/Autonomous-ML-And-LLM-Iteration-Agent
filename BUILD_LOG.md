...
## Sprint 4: v0.6, DLModelTarget vision. Sun 2026-09-13 to Tue 2026-09-22, release Sun 2026-09-27

*(Re-planned 2026-09-12. The Aug 10-15 dates never ran: sprint 3's code was complete Aug 11 and its release waited on the certification run until Sep 9. Re-dated again 2026-09-14: Tony's call to put the data linker before the target added three build days, so the release moved one Sunday.)*
...
| Mon Sep 14 | **The data linker, PR 1 of 3: the rules ladder and the canonical data folder.** `iterate run --data <folder>` (or two folders, the user's split) works out how images link to labels by rules alone (wrapper folders collapsed, a table joined to the images on an exact key, one-hot columns, a split column, class folders with or without a train and test pair), refuses what the rules cannot prove with the reason and the flags that would settle it, and writes `.iterate/data/<name>/` with `raw_files/`, `train/`, `holdout/`, the two CSVs and the plan | `schemas/link.py`, `adapters/data/linking.py`, `adapters/data/workspace.py`, the folder lane in `run`, tests | done |
| Tue Sep 15 | **The Linker, PR 2 of 3.** The sixth specialist: when the ladder refuses a folder for want of a choice (no rule fits, two tables tie, two key columns disagree, a label table no key resolves), a capped listing of the folder with the join share the harness measured for every column goes to the model, and it answers one tool call: the table by number, the key and label columns by name, how the key names the file from a closed list. The harness rebuilds and measures the frame through the same function a rules plan goes through and reads the task from the labels; the pause becomes a conversation, yes, no, or a correction in plain English, five rounds at most, `--yes` in scripts; an accepted plan is remembered so a folder never goes through the Linker twice | `core/linker.py`, prompts.yaml `linker` keys, `plan_from_choice`, the pause loop, tests, a live pass on gemma4:12b | done (built Mon Sep 14) |
| Wed Sep 16 | **The monitor, PR 3 of 3.** Seven deterministic checks on the rows the folder will be written with, before the pause asks for a yes: coverage both ways, duplicate and conflicting labels, byte twins across the split, lookalikes at a coarse hash, a second label source, class balance, id-like columns that span the split; a typed report printed under the link block, saved as `monitor.json` beside the plan, and a counts-only brief with a seat in the supervisor's data summary (filled on Day 5). Reports, and never a relabel: a byte copy of a training image is left out of a holdout we make, and comes out of a holdout the person gave when they answer `drop` at the pause (Tony's call). The label audit and the vision spot check follow the target | `schemas/monitor.py`, `adapters/data/monitor.py`, `workspace.sides`, `linking.table_rows`, the pause, tests | done (built Mon Sep 14 to Tue Sep 15) |
| Thu Sep 17 | `DLModelTarget`: probe baseline, a typed recipe runner for `run()` with a budget planned before the first step, code job and scoring, meta.json with family vision, and the `prepare_images` step that hands it byte-named copies. Vision ceiling sweep through the target's own `run()`, twelve recipes, no LLM, run overnight on MPS | `targets/dl.py`, `adapters/data/images.prepare_images`, `evals/vision_ceilings.py`, the family key in the corpus and the runner, two `dataset.toml`, headroom numbers | done (built Wed Sep 16): Flowers102 0.892 to 0.974, EuroSAT 0.899 to 0.986 |
| Fri Sep 18 | The code path: vision preamble (device pick, torch seeding, cached tensors, helpers, worked example), floor cell, GPU memory freed between cells, CLI dispatch with `--data <folder>` through the linker and e2b refusal, prompts.yaml vision keys for all four roles, the vision lever ladder and code markers in the supervisor, first live MPS run on gemma4:12b. Torch is installed at the START of a vision run when it is missing, under the same one-time consent as install-on-demand and with no time cap, never mid-cell; and the on-demand installer refuses to install or change `torch` and `torchvision` during a run, since that would replace the runtime under a live kernel (Tony's call, 2026-09-13). Amended 2026-09-16 at the Day 5 gate: the copy of `monitor.json` into the run folder and the supervisor reading the brief move here, since both need the run the CLI starts; the eval runner's dev gate for vision cells lifts with the CLI guard, and a test ties the two; a cell guard refuses agent code that names `.iterate`, `raw_files`, `link.json`, `monitor.json` or a `holdout/` folder, and the Critic's brief names the workspace; the kernel preamble sets the MPS pool cap beside the fallback flag and its fit helper counts from the top of the cell; and the installer rule gets its seam: `coder._run_with_autoinstall` refuses torch and torchvision by name, and a constraints file pinning the installed versions goes to every install, because torchvision 0.29.0 pins torch 2.14.0 exactly and a name filter alone would let a dependency swap it. And torch never shares a process with a lightgbm fit, measured 2026-09-16: on macOS the two OpenMP runtimes crash or hang in one process | wiring, prompts, tests, first smoke run | |
| Sat Sep 19 | Live runs and what only running finds: guards for the failure modes a 12B produces writing torch. Notebook check that training curves render. LIMITATIONS rows: MPS nondeterminism, e2b, floor-model torch reliability | fixes, tests, docs | |
| Sun Sep 20 | RTX 4050 validation, the public claim: install over WSL2, same example, CUDA versus MPS epoch time, VRAM peak, an OOM run to prove capture. Example READMEs, examples index, RESULTS regenerated | validation notes, docs | |
| Mon Sep 21 | Carry-ins 1 to 5 and 9, one PR: cached-baseline floor, `prompts.yaml` labels, prompt dead-lever guard (replayed on STS-B first), small-slice-then-promote measured as calls per iteration before and after, sized free-text cap, tweet_irony tracked | wiring, tests, measurements | |
| Tue Sep 22 | Carry-ins 6 to 8: the eval runner fix, the v3 tuned-ensemble axis, the seventh technique in both prompt sweeps with STS-B and irony re-measured. Then the release gate per the standing checklist: README and LIMITATIONS sync, version bump, launch drafts | `evals/tuning.py`, sweep v2, release PR | |
| Sun Sep 27 | Tony's certification run on the vision example with measured headroom (sequential, Ollama restarted first), then **release v0.6.0** | v0.6.0 out | |
...
### 2026-09-16 | Sprint 4 Day 5 | DLModelTarget, and the ladder that measures its ceiling

**Task:** the plan's original Day 2, after the three linker PRs. The third target on the same loop, and the no-LLM sweep that says how much headroom each vision dataset holds. The CLI does not start it yet; that is the code-path day. The sweep ran the night the code was built, so its numbers are in this PR.
...
**Headroom.** Measured overnight on an Apple M5 with MPS, each recipe once, 28 minutes for both datasets. All 12 recipes scored on both, and none was trimmed by the budget: every fine-tune ran the epochs it planned.
...
**Not in this PR, by design:** JSON annotations, headerless text lists, labels in file names, multi-label columns, nested class folders, and any layout where two plans tie. Those are the Linker's territory tomorrow. The run itself waits for the vision target on Day 5: until then a folder run links, confirms, writes the folder and stops with a line saying so, and the same guard stops a plain CSV of image paths, which before this PR went into the tabular loop on file names.
