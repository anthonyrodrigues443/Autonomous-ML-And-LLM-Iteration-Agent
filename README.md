...
| v0.6 | **`DLModelTarget`: an image folder or a CSV of image paths runs the same loop a table does.** A plain CNN baseline, then briefed experiments that fine-tune pretrained backbones or write their own torch code, for classes and numbers, scored on a sealed holdout. Run live on Apple MPS with a local 12B. Not yet run on CUDA: an RTX 4050 check over WSL2 follows the release. | shipped |
