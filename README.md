...
Useful flags: `--train` + `--holdout` instead of `--data` (bring your own split: the holdout is sealed exactly as given; code path only, the `--spec` lane keeps `--data`), `--max-iterations`, `--patience`, `--until` (wall-clock bound), `--notebooks best|all|none`, `--compute local|e2b`, `--install/--no-install` (package-install consent), `--think` (reasoning mode for the coder, Ollama only), `--fresh` (archive memory, start a new chapter), `--plain` (classic output instead of the interactive UI), `--requests-per-hour` (how many predictions an hour the winner will serve, default 1,000; the end of the run prices the winner at that rate), `--cloud aws|gcp|azure` (price the winner on one cloud; omitted, the run compares AWS and GCP and shows the cheapest first), `--region` (the cloud region the prices are for, with `--cloud`), `--serving-budget` (dollars a month you can spend serving the winner at that rate; a hard wall, not a weight on the score: a model that costs more to serve is never banked or recommended, and on an image run never briefed either; among the models that fit, the best score wins; a budget under even the baseline's price is refused before the run starts; type `budget $80` or `no budget` mid-run to move it), `--spec` (the v0.1 allow-list path, kept as the fast lane). Prompt runs: `--task` (switches to prompt iteration), `--prompt-file` (your current prompt as the baseline), `--target-model` / `--target-backend` (the model whose prompt is tuned, separate from the one driving the run), `--loop-holdout` (records per candidate during the search). Image folders: `--labels` (the table that holds the labels, when the rules should not pick one), `--key` (the column in `--labels` that names each image), `--yes` (accept a link the rules could not fully prove, or one the Linker proposed, without the pause). Full reference: `iterate run --help`

**Where things land:** `.iterate/runs/<run_id>/best.ipynb` (the runnable winner), `meta.json` + `train.csv` + `holdout.csv` beside it (the bytes the session read; the holdout has no labels in it), `notebooks/` (with `--notebooks all`), `best.json` (config + score sidecar, and a `serving` block: what the winner costs a month to serve at your request rate, on the cheapest machine or API that can, with where each number came from; the same line prints after `best:` at the end of the run, says which price list it used, and with `--serving-budget` says whether the winner is within it), `prompts.yaml` on a prompt run (every version with its score, the best marked, and the same `serving` block), `best_model.pt` on an image run (the winning network, read-only). Prices come from the clouds' own lists: `iterate prices refresh` fetches them into `~/.cache/iterate/prices/` (AWS's list is about 300 MB and is streamed and reduced once; Azure's open API is walked page by page; GCP's catalog needs a key, so its shipped rows stand) along with timm's published table of network sizes, `iterate prices show` says what the Pricer would read right now, and offline with no cache the snapshot shipped in the package stands in, dated. API model prices have no feed anywhere and stay a dated snapshot. `.iterate/` is git-ignored by a `.gitignore` written when the folder is first made, since a run folder holds a copy of your training rows. Code-path winners ship as notebooks by design; `--spec` winners also save `best_model.joblib`. `--output` moves the model file, and `best.json` goes beside it.

**The serving budget is a wall.** With `--serving-budget 50`, the run never banks or recommends a model that would cost more than $50 a month to serve at your `--requests-per-hour`, and on an image run never briefs one. Before the loop the baseline itself is priced (the plain CNN at its size, the default boosting pipeline on your feature count, the starting prompt on the model under test), and a budget under even that is refused before a folder is made, with the price, the budget and, when the baseline runs on a machine rather than an API, the cheapest machine that could hold it. On an image run every entry on the supervisor's ready line is priced before the brief: the entries over the budget move under their own heading with their monthly price, and a brief naming one anyway is rejected with the first affordable entry named, top down, never the cheapest. When nothing on the line fits, the line says so and the block lists every entry with its price; the run then stops on the supervisor's own stop rule or runs out of patience, and the end of the run lists what the wall turned away. On every family each finished experiment is priced from what it actually did (the recipe an image session submitted, the estimator classes a table session used, the tokens a prompt spent per record), and one over the budget is stamped with its price on the run's history and can never become the best, whatever it scored. The objective stays the score: the budget decides what is in the running, never the ranking, and nothing is rewarded for being cheaper. Type `budget $80` mid-run to move the wall from the next brief on, or `no budget` to take it down; only you can move it. Prices are the Pricer's estimates from the same table the serving line uses, so a network the table cannot size is not walled, and the serving line says so.
...
│   ├── serving           # the Pricer: what the winner costs to serve, with a basis for every number
│   ├── prices            # the clouds' own price lists, fetched, reduced and cached; timm's network sizes
