...
| v0.6 | **`DLModelTarget`: an image folder or a CSV of image paths runs the same loop a table does.** A plain CNN baseline, then briefed experiments that fine-tune pretrained backbones or write their own torch code, for classes and numbers, scored on a sealed holdout. GPU compatible: Apple (MPS) and NVIDIA (CUDA), with CPU as the fallback. Run live with a local 12B. | shipped |
