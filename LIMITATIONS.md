...
| The v0.2 coder + supervisor prompts live on the open PR #43 branch, not yet merged — `main` still runs the pre-rewrite prompts. | **v0.2 (merge call)** | Resolved in substance: both prompts were rewritten via research forges (RESEARCH_LOG 2026-06-08 and 2026-06-09) and validated live (run 5d56268c: 10/10 experiments ≥ 0.60, best 0.6325, zero monolithic cells). Each rewrite sits in its own commit for one-commit revert; the remaining gate is the merge decision. |
