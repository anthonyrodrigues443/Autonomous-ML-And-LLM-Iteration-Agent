...
|---|---|---|
| Closed metric set: the 12 rows registered in `core/scoring.py` (`accuracy, f1, precision, recall, roc_auc, average_precision, log_loss, brier` / `rmse, mae, mse, r2`). Anything else still raises "unknown metric". | **widened v0.4**; closed by design | Was a hardcoded 8-metric panel; now a registry where each row carries its own task, direction and probability requirement, so adding a metric is one line. Kept closed on purpose: `direction` drives what banks as best and when the run stops, so a metric chosen at runtime could silently invert the loop. The agent picks *which* metric; the registry owns *which way*. |
| Probability metrics are computable but not yet reachable from a live run: `score()` accepts `y_proba`, but the code-gen path has no way to capture `predict_proba` output. | **v0.4** (contract pending) | ROC-AUC / PR-AUC / log-loss / Brier all score correctly in-process as of the metric layer. What is missing is the `probabilities.csv` artifact + validator so the coding agent can emit probabilities at all. Until that lands, selecting a probability metric on a real run has nothing to score. |
| `--average` is not exposed on the CLI, so `f1`/`precision`/`recall` averaging cannot be chosen by the user. | **v0.4** (flag pending) | `core.scoring.resolve_average` implements binary/micro/macro/weighted, and the default preserves the previous behaviour exactly (binary on two classes, macro otherwise). The flag lands with `--metric` going optional, since both touch the same option block. ROC-AUC keeps macro on multiclass: sklearn's supported set there is narrower and varies across the supported version range. |
